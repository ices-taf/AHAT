...
params:
  ...
  series: must_supply
  pagetitle: must_supply
---

---
pagetitle: `r params$pagetitle`
---
...
txt_trend3 <- switch(
  ...
  lognormal = paste0(
    ...
    ", with d or q denoting values below the limit of detection or quantification and ",
...
<li>Matrix: `r info$matrix_name`</li>
<li class = "gap">Group: `r replace_na(as.character(info$AMAP_group), "")`</li>
...
change_txt <- function(type = c("whole", "recent")) {
  ...
  change <- switch(type, whole = "overall", recent = "last 20 years")
  change <- contrasts[change, "Change"]
  change <- abs(round(100 * (exp(change) - 1), 1))
  paste0(
    ...
    " ", format_p(p), 
    if (p < 0.05) paste0(" by an estimated ", change, "%.") else "."
  )
}
...
if (wk$p_overall >= 0.05) {
  ...
  trend_description <- paste0(
    ...
    change_txt("whole"), 
    if (assessment$contrasts["whole", "start"] < wk_recent) paste0(" ", change_txt("recent"))
  )
}
```
...
The lowest detectable annual increase in the time series (two-sided test, power = 80%, size = 5%) is `r wk$dtrend_1`%. `r if (!wk_seq) paste0("Had every year been sampled (so there were no gaps in the time series), the lowest detectable annual increase would have been ", wk$dtrend_2, "%.")`  The lowest detectable annual increase with 10 years of `r if (!wk_seq) "annual"` monitoring is `r wk$dtrend_3`%.  The lowest detectable annual decrease with 10 years of `r if (!wk_seq) "annual"` monitoring is `r round(100 * wk$dtrend_3 / (100 + wk$dtrend_3), 1)`%.

Given the variability in the data, `r wk$dyear` years of `r if (!wk_seq) "annual"` monitoring would be required to detect an annual increase of 5% with 80% power (two-sided test, size = 5%).

The power of the time series to detect an annual increase of 5% is `r wk$dpower_1`% (two-sided test, size = 5%).  `r if (!wk_seq) paste0("Had every year been sampled, the power would have been ", wk$dpower_2, "%")`.  With 10 years of `r if (!wk_seq) "annual"` monitoring, the power would be `r wk$dpower_3`%.
